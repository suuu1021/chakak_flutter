...
import '../../dtos/photographer/photographer_profile_dto.dart';
...
abstract class PhotographerProfileRepository {
  ...
  Future<void> updateProfile(String id, PhotographerProfileFormDto dto);
...
class PhotographerProfileRepositoryImpl
  ...
  Future<PhotographerProfile> createProfile(
    ...
    try {
      ...
      print(
          '[DEBUG] createProfile DioException: ${e.response?.statusCode} ${e.message}');
  ...
  @override
  Future<void> updateProfile(String id, PhotographerProfileFormDto dto) async {
    // 반환 타입 변경
    try {
      ...
      final response =
          await _dio.put('/api/photographers/$id/update', data: body);
      ...
      print(
          '[DEBUG] updateProfile DioException: ${e.response?.statusCode} ${e.message}');
  ...
  Future<PhotographerProfile?> getProfile(String id) async {
    try {
      ...
      print(
          '[DEBUG] getProfile DioException: ${e.response?.statusCode} ${e.message}');
  ...
  Future<PhotographerProfile?> getMyProfile() async {
    try {
      ...
      if (responseBody is Map<String, dynamic>) {
        ...
        } else if (responseBody.containsKey('body') &&
            responseBody['body'] is Map<String, dynamic>) {
          profileData = responseBody['body'] as Map<String, dynamic>;
        } else if (responseBody.containsKey('response') &&
            responseBody['response'] is Map<String, dynamic>) {
      ...
      if (profileData == null) {
        print(
            '[DEBUG] 파싱할 프로필 데이터(data, body, response)를 찾을 수 없거나 형식이 맞지 않습니다: $responseBody');
        return null;
      }
      ...
      print(
          '[DEBUG] getMyProfile DioException: ${e.response?.statusCode} ${e.message}');
      if (e.response?.statusCode == 404) {
  ...
  Future<void> deleteProfile(String id) async {
    try {
      ...
      print(
          '[DEBUG] deleteProfile DioException: ${e.response?.statusCode} ${e.message}');
  ...
  Future<void> updateProfileStatus(String id, String status) async {
    try {
      final response = await _dio
          .put('/api/photographers/$id/status', data: {'status': status});
      if (response.statusCode != 200) {
      ...
      print(
          '[DEBUG] updateProfileStatus DioException: ${e.response?.statusCode} ${e.message}');
